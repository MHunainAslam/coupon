    ...
    return (
        ...
        <div className=" bg-white row p-2 my-2 mx-2 coupon ">
            ...
            <div className="col-md-7">
                ...
                <p className='mb-0'>{coupon?.desc}</p>
                <div className='d-md-flex justify-content-between mb-0'>
                    <small>{coupon?.expdate}</small>
                    {is_ico
                        &&
            ...
            <div className="col-md-3 text-end m-auto">
                <Link href={`${coupon?.url || ''}`} onClick={() => { window.open(`/store/${data?.query?.slug}/${coupon?.coupon_id}`) }} className={`p-2   button button-${coupon?.type === 'code' ? 'secondary' : 'primary'}`}  >
                    {coupon?.type === 'code' ? 'Show Code' : 'Show Deal'}
                </Link> <br />
                <small >{coupon?.updatedate}  </small>
            </div>
        </div >
